...
public class ViewCommunicationBookFrame extends JFrame implements CrossButtonListener, ListSelectionListener
{
...
private CommunicationBookModel communicationBookModel;
private JTable communicationBookTable;
private JScrollPane communicationBookScrollPane;

public ViewCommunicationBookFrame()
{
initComponent();
setTitle("View Communication Book");
...
{
communicationBookModel=new CommunicationBookModel();
communicationBookTable=new JTable(communicationBookModel);
communicationBookTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
communicationBookTable.getColumnModel().getColumn(0).setPreferredWidth(20);
communicationBookTable.getColumnModel().getColumn(1).setPreferredWidth(30);
communicationBookTable.getColumnModel().getColumn(2).setPreferredWidth(30);
communicationBookTable.getColumnModel().getColumn(3).setPreferredWidth(30);
communicationBookTable.getColumnModel().getColumn(4).setPreferredWidth(30);
communicationBookTable.getSelectionModel().addListSelectionListener(this);
communicationBookScrollPane=new JScrollPane(communicationBookTable,ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
}
...
communicationBookScrollPane.setBounds(60,20,530,200);
...
add(communicationBookScrollPane);

disableAll();
communicationBookTable.setRowSelectionInterval(0,0);
}
...
{
int selectedRowIndex=communicationBookTable.getSelectedRow();
ReferenceBookInterface referenceBook=communicationBookModel.get(selectedRowIndex);
