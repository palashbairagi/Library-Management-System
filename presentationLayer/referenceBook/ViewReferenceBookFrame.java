...
public class ViewReferenceBookFrame extends JFrame implements CrossButtonListener, ListSelectionListener
{
...
private ReferenceBookModel referenceBookModel;
private JTable referenceBookTable;
private JScrollPane referenceBookScrollPane;

public ViewReferenceBookFrame()
{
initComponent();
...
{
referenceBookModel=new ReferenceBookModel();
referenceBookTable=new JTable(referenceBookModel);
referenceBookTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
referenceBookTable.getColumnModel().getColumn(0).setPreferredWidth(20);
referenceBookTable.getColumnModel().getColumn(1).setPreferredWidth(30);
referenceBookTable.getColumnModel().getColumn(2).setPreferredWidth(30);
referenceBookTable.getColumnModel().getColumn(3).setPreferredWidth(30);
referenceBookTable.getColumnModel().getColumn(4).setPreferredWidth(30);
referenceBookTable.getSelectionModel().addListSelectionListener(this);
referenceBookScrollPane=new JScrollPane(referenceBookTable,ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
}
...
referenceBookScrollPane.setBounds(60,20,530,200);
...
add(referenceBookScrollPane);

disableAll();
referenceBookTable.setRowSelectionInterval(0,0);
}
...
{
int selectedRowIndex=referenceBookTable.getSelectedRow();
ReferenceBookInterface referenceBook=referenceBookModel.get(selectedRowIndex);
